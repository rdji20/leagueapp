...
export const RecentMatchesList = ({ navigation, userProp, matches, users }) => {
    ...
    const findRecords = (uid) => {
        return `${users.find((element) => element.userId === uid)["wins"]}-${
            users.find((element) => element.userId === uid)["loses"]
        }`;
    };
        ...
        return matches.slice(0, 3).map(function (match) {
            return (
                <RecentMatches
                    key={match.matchId}
                    records={[
                        findRecords(match.data[0].uid),
                        findRecords(match.data[1].uid),
                    ]}
                    players={[
                        findDisplayName(match.data[0].uid),
                        findDisplayName(match.data[1].uid),
                    ]}
                    score={[match.data[0].score, match.data[1].score]}
                    teams={[match.data[0].team, match.data[1].team]}
                    times={match.timeStamp}
                />
            );
        });
    };
    ...
    return matches.length > 0 ? (
        ...
        <View style={styles.matchContainer}>
            <MaterialCommunityIcons
                name="stadium"
                style={{ color: "white", fontSize: 30, marginBottom: 8 }}
            />
            <Text style={styles.noMatches}> No matches Yet! </Text>
            <Text style={styles.noMatchesDescription}>
                {" "}
